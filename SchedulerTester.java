...
public class SchedulerTester {
    ...
    private static final String[] GOALS = {
            "max wait", "avg wait", /* "efficiency", */ "combined", "deadlines" };
    ...
    public static void main(String[] args) {
        ...
        maxWaitTimeGoals = Arrays.asList(
                new Integer[] { 1600, 2000, 2500, 3500, 5500 }); // , 10000, 20000});
        avgWaitTimeGoals = Arrays.asList(
                new Integer[] { 240, 300, 420, 600, 900 }); // , 1500, 2500});
        combinedGoals = Arrays.asList(
                new Integer[] { 2275, 2350, 2500, 2700, 2900 }); // , 3000, 3400});
        int percentageToNumJobs = (int) Math.ceil(numJobs / 100);
        List<Integer> deadlineGoals = Arrays.asList(new Integer[] {
                85 * percentageToNumJobs, 80 * percentageToNumJobs,
                75 * percentageToNumJobs, 70 * percentageToNumJobs,
         ...
         * 21000, 32000, 48000}); avgWaitTimeGoals = Arrays.asList(new Integer[] {1500,
         * 1600, 1700, 1800, 1900, 2100, 2500}); combinedGoals = Arrays.asList(new
         * Integer[] {6700, 7000, 7500, 8100, 8800, 9500, 10500});
         */
        ...
        for (String goal : goals.keySet()) {
            ...
            System.out.println("That earns: " + score + "/" + maxScore +
                    " points!\n");
            totalScore += score;
            totalMax += maxScore;
        }
        ...
        System.out.println("All tests completed: " + pointsForFinishing +
                " points");
    ...
    // from the run. Stats are returned as: [maxWaitingTime, totalWaitingTime]
    private static long runTest(String property, int numJobs, int minJobLength,
            int maxJobLength, boolean verbose) {
        ...
        LinkedBlockingQueue<Job> generatorToScheduler = scheduler.getOutgoingQueue();
        ...
        for (int i = 0; i < numJobs / 2; i++) {
            int jobLength = (int) i * (maxJobLength / 2 - minJobLength) / jobsThisPart +
                    minJobLength;
        ...
        for (int i = 0; i < jobsThisPart; i++) {
            int jobLength = (int) i * (maxJobLength / 20 - minJobLength) / jobsThisPart +
                    minJobLength;
            lengths.add(jobLength);
        }
        ...
        Thread jobGenerator = new Thread(() -> {
            ...
            for (; i < numToPrime;) {
                 ...
                 * maxJobLength + 1); Job job = new Job(jobLength, ids.get(i));
                 */
                ...
                try {
                    ...
                    System.err.println("Putting a job in the queue to reach the " +
                            "scheduler (while priming) got interrupted!");
                }
            }
            System.out.println("!!!!!!! Done priming the simulation!!!!!!!!");
            // now generate jobs for the scheduler, waiting between each one you
            // create.
            for (; i < numJobs;) {
                /*
                 * int jobLength = ThreadLocalRandom.current().nextInt(minJobLength,
                ...
                int j = i;
                // add the job to the scheduler in a separate thread so that our
                // processing can't be blocked by an unnaturally small BlockingQueue
                // headed into the scheduler.
                Thread giveToScheduler = new Thread(() -> {
                    try {
                        ...
                        System.err.println("Adding a job in the queue to reach the " +
        ...
        assert completed.size() == numJobs
                : "The number of completed jobs doesn't match the number created!";
        ...
        return (new long[] { cpu.getMaxWaitTime(), cpu.getAverageWaitTime(),
                /* cpu.getEfficiency(), */ cpu.getMaxWaitTime() +
                        2 * cpu.getAverageWaitTime(),
